      ...
      [`(,op ,src ,dest)
       (let-values ([(type_src sym_src) ((assign-homes var_locs) src)]
         ...
         (cond
           [(and (eq? 'var type_src)
                 (eq? 'var type_dest))
            `(,op (deref rbp ,(- (* 8 (index-of var_locs sym_src)))) (deref rbp ,(- (* 8 (index-of var_locs sym_dest)))))]
           [(eq? 'var type_src)
            `(,op (deref rbp ,(- (* 8 (index-of var_locs sym_src)))) ,dest)]
           [(eq? 'var type_dest)
            `(,op ,src (deref rbp ,(- (* 8 (index-of var_locs sym_dest)))))]))]
      [`(,type ,sym)
       (values type sym)]
      [`(retq) e]
      )))
